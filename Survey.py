...
class Survey():
    def __init__(self, cfg):
        if "id" in cfg:
            ...
            logging.error("Survey id missing from 'surveys' object in survey_config.json")
            self._id = None
        if "save_dir" in cfg:
            self._save_dir = cfg["save_dir"]
            logging.debug("Loaded save directory")
        else:
            logging.warn("No save directory (key: 'save_dir') specified in 'surveys' object in survey_config.json. Default will be used without error")
            self._save_dir = None
        if "rename" in cfg:
            self._rename = cfg["rename"]
            logging.debug("Loaded file rename")
        else:
            logging.warn("No file rename (key: 'rename') specified in 'surveys' object in survey_config.json. File will not be renamed.")
            self._rename = None
    ...
    def rename(self, file_name, downloads_dir):
        ...
        for r in self.getRename():
            logging.debug(f"Replacing {r['replace_pattern']} in file with {r['replace_with']}")
            new_name = replace(new_name, r["replace_pattern"], r["replace_with"])
    ...
    def deepCopy(self):
        ...
        if not self.isValid():
            logging.error(f"{self} is not a valid Survey. Can't create deep copy.")
            return None
        cfg = {"id": self._id}
        if self._save_dir:
        ...
        logging.debug(f"New survey created: {newSurvey}. IsValid: {newSurvey.isValid()}")
        return newSurvey
